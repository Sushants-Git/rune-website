import { PROGRAMS } from "./ProgramIcons";

/* The row tile, reproduced from IconTile in Sources/Rune/SwitcherPalette.swift.

   The marks themselves are lifted verbatim from AgentIcon.swift, which is why
   they carry their own brand colours rather than currentColor. Several are
   near-black and were drawn for light backgrounds, so — exactly as the app does
   — a recognised agent gets a white tile of its own, the way an app icon sits in
   a launcher. A row running a recognised program gets the same treatment, from
   the same 24 marks the app carries. Only a genuinely unrecognised shell falls
   through to the dim tile and an SF Symbol. */

export default function AgentIcon({ agent, program }) {
  const art = MARKS[agent] ?? PROGRAMS.find((p) => p.id === program)?.mark;

  if (!art) {
    return (
      <span className="tile tile--shell">
        <TerminalSymbol />
      </span>
    );
  }

  return <span className="tile tile--art">{art}</span>;
}

const MARKS = {
  claude: (
    <svg viewBox="2 5 20 15" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
      <path
        fill="#D97757"
        d="M20.998 10.949H24v3.102h-3v3.028h-1.487V20H18v-2.921h-1.487V20H15v-2.921H9V20H7.488v-2.921H6V20H4.487v-2.921H3V14.05H0V10.95h3V5h17.998v5.949z"
      />
      <rect x="6" y="8.102" width="1.488" height="2.847" fill="black" />
      <rect x="16.51" y="8.102" width="1.49" height="2.847" fill="black" />
    </svg>
  ),

  codex: (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="#fff"
        d="M19.503 0H4.496A4.496 4.496 0 0 0 0 4.496v15.007A4.496 4.496 0 0 0 4.496 24h15.007A4.496 4.496 0 0 0 24 19.503V4.496A4.496 4.496 0 0 0 19.503 0z"
      />
      <path
        fill="#4B63FF"
        d="M9.064 3.344a4.578 4.578 0 0 1 2.285-.312c1 .115 1.891.54 2.673 1.275.01.01.024.017.037.021a.09.09 0 0 0 .043 0 4.55 4.55 0 0 1 3.046.275l.047.022.116.057a4.581 4.581 0 0 1 2.188 2.399c.209.51.313 1.041.315 1.595a4.24 4.24 0 0 1-.134 1.223.123.123 0 0 0 .03.115c.594.607.988 1.33 1.183 2.17.289 1.425-.007 2.71-.887 3.854l-.136.166a4.548 4.548 0 0 1-2.201 1.388.123.123 0 0 0-.081.076c-.191.551-.383 1.023-.74 1.494-.9 1.187-2.222 1.846-3.711 1.838-1.187-.006-2.239-.44-3.157-1.302a.107.107 0 0 0-.105-.024c-.388.125-.78.143-1.204.138a4.441 4.441 0 0 1-1.945-.466 4.544 4.544 0 0 1-1.61-1.335c-.152-.202-.303-.392-.414-.617a5.81 5.81 0 0 1-.37-.961 4.582 4.582 0 0 1-.014-2.298.124.124 0 0 0 .006-.056.085.085 0 0 0-.027-.048 4.467 4.467 0 0 1-1.034-1.651 3.896 3.896 0 0 1-.251-1.192 5.189 5.189 0 0 1 .141-1.6c.337-1.112.982-1.985 1.933-2.618.212-.141.413-.251.601-.33.215-.089.43-.164.646-.227a.098.098 0 0 0 .065-.066 4.51 4.51 0 0 1 .829-1.615 4.535 4.535 0 0 1 1.837-1.388zm3.482 10.565a.637.637 0 0 0 0 1.272h3.636a.637.637 0 1 0 0-1.272h-3.636zM8.462 9.23a.637.637 0 0 0-1.106.631l1.272 2.224-1.266 2.136a.636.636 0 1 0 1.095.649l1.454-2.455a.636.636 0 0 0 .005-.64L8.462 9.23z"
      />
    </svg>
  ),

  opencode: (
    <svg viewBox="0 0 240 300" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
      <path d="M180 240H60V120H180V240Z" fill="#CFCECD" />
      <path d="M180 60H60V240H180V60ZM240 300H0V0H240V300Z" fill="#211E1E" />
    </svg>
  ),
};

/* Stand-ins for the two SF Symbols the app falls back to. */

function TerminalSymbol() {
  return (
    <svg viewBox="0 0 16 16" fill="none" aria-hidden="true">
      <rect x="1.4" y="2.6" width="13.2" height="10.8" rx="2.4" stroke="currentColor" strokeWidth="1.3" />
      <path
        d="m5.3 6.4 1.9 1.6-1.9 1.6M8.9 10.2h2.2"
        stroke="currentColor"
        strokeWidth="1.3"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
}

